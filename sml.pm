...
sml->add_to_class('column_means', \&{'column_means'});
  ...
  # Versión corregida de column_stdevs
  sub column_stdevs {
  ...
  # Function To Standardize a Dataset using tensor operations
...
sml->add_to_class('count_labels', \&{'count_labels'});
  ...
  sub confusion_matrix{
      ...
      # Step 1: One-hoy encode the actual and predicted arrays
      my $num_classes     =  $actual->max->asscalar + 1;
      my $actual_one_hot  =  mx->nd->one_hot($actual, $num_classes)->T;
      my $predicted_one_hot = mx->nd->one_hot($predicted, $num_classes);

      # Step 2: Compute confusion matrix
      # Matrix multiplication: (actual_one_hot T)* predicted_one_hot
      return mx->nd->arange(stop => $num_classes), mx->nd->dot($actual_one_hot, $predicted_one_hot);
